...
var fivedaysForecastUrl = "http://api.openweathermap.org/data/2.5/forecast?q=" + searchChar + "&appid=" + apiKey + "&units=imperial";
...
document.addEventListener("submit",function handleFormSubmit(event) {
  ...
  //   window.alert("You've already searched it!")
  //   return;
  // }
    historyApend();
...
function historyApend(){
    ...
    historyListItemEl.addEventListener("click", function(){
        getWeather(localStorage.getItem(cityInput))
...
function getWeather(cityInput){
    ...
    var fivedaysForecastUrl = "http://api.openweathermap.org/data/2.5/forecast?q=" + cityInput + "&appid=" + apiKey + "&units=imperial";
    ...
    .then(function(data){
        for (var i = 3; i<36; i+=8) {
          var ddate = data.list[i].dt_txt.split(" ")[0];
          var dIcon = data.list[i].weather[0].icon;
          var iconUrl = "http://openweathermap.org/img/wn/" + dIcon + "@2x.png";
          var dtemp = data.list[i].main.temp;
          var dhu = data.list[i].main.humidity;
          var dwindS = data.list[i].wind.speed;
          var z = document.createElement("div");
          z.innerHTML = ddate + "<br />"+ "<img src="+iconUrl+"><br />Temperature: " +dtemp+"°F"+"<br />Humidity: " + dhu +" %"+"<br />Wind Speed: " +dwindS +" mph";
          z.setAttribute("class", "card col-2")
          document.getElementById("fivedays").appendChild(z);
          
          // var dIcon = data.list[i].weather[0].icon;
          // var iconUrl = "http://openweathermap.org/img/wn/" + dIcon + "@2x.png";
          // document.getElementById("weather-icon").setAttribute("src", iconUrl);
          // var dtemp = data.list[i].main.temp;
          // document.getElementById("temperature").textContent = dtemp;
          // var dhu = data.list[i].main.humidity;
          // document.getElementById("humidity").textContent = dhu;
          // var dwindS = data.list[i].wind.speed
          // document.getElementById("wind-speed").textContent = dwindS;
